...
# also silence numpy numerical warnings (overflow/invalid ops)
try:
    import numpy as np
...
# if warnings still leak through logging, hard-disable logging with caution
...
def scan_existing_outputs(output_dir, base_name):
    """
    Scan combined output and shards.
    Returns existing_idx(set), existing_q(set) for checkpoint resume and duplicate skipping.
    """
...
def merge_all_shards(output_dir, base_name, remove_shards=True):
    """rank 0: merge *.shard*.jsonl (+ existing combined), dedupe by idx, and write {base}.jsonl"""
...
def init_distributed_if_needed():
    """
    Initialize with env:// for torchrun; otherwise single-process.
...
# custom step-by-step sampling matching HF do_sample+top_p+temperature
@torch.no_grad()
def sample_with_tracking(
    ...
    """
    Returns:
      generated_ids: token ids of newly generated segment ([T])
    ...
    # prefill full prompt to build KV cache without changing generation logic
...
def worker(args, rank, world_size, local_rank, device):
    ...
    shard_file_base = os.path.join(output_dir, f"{base_name}.shard{rank:03d}_base.jsonl")
    # scan existing outputs (combined + all shards) for resume/skip
    ...
    sampler_base = DistributedSampler(
        list(range(args.budget_num)),
        num_replicas=world_size,
        rank=rank,
        shuffle=False,
        drop_last=False
    )
    sampler_base.set_epoch(0)
    raw_idx_base = list(iter(sampler_base))
    # deduplicate to avoid padding duplicates
    seen_base, my_indices_base = set(), []
    for i in raw_idx_base:
        if i < N and i not in seen_base:
            seen_base.add(i)
            my_indices_base.append(i)

    import time
    time.sleep(0.2*rank)
    print(f"[rank {rank}] shard_file = {shard_file_base}")
    print(f"[rank {rank}] will process {len(my_indices_base)} base items")
    print("=" * 80)
    dist.barrier()

    pbar = tqdm(total=len(my_indices_base), desc=f"Rank {rank} DP Inference", position=rank, leave=True)

    for i in my_indices_base:
        q = questions[i]
        qtext = q.get("problem", "")

        # checkpoint resume: skip already existing samples
        messages = [
        ...
        with open(shard_file_base, 'a', encoding='utf-8') as f:
    ...
    pbar.close()
    print(f"[rank {rank}] ✅ Done. Shard saved to {shard_file_base}")
    print("=" * 80)
...
def evaluate_and_save(args, combined_file):
    ...
    def _extract_first_text(gen):
        """Handle mixed structures; use first text for length stats while grading checks all candidates."""
    ...
    def _extract_all_texts(gens):
        ...
        for g in gens:
            if isinstance(g, str):
                ...
                # take first nested string from list item
    ...
    if pass_at_k_vals:
        ...
        metrics[f"pass@{args.k}"] = acc  # pass degenerates to accuracy for single sample
